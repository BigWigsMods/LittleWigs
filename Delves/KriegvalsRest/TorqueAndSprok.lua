...
mod:SetEncounterID(3140) -- in Kriegval's Rest
...
function mod:OnBossEnable()
	-- note: this module should be kept in sync with Torque and Sprok from The Underkeep.
	-- the only things that are different are the encounter id and the mob ids.

	-- Torque Clankfire
	self:Log("SPELL_CAST_START", "LongFuseMissiles", 1215033)
	self:Log("SPELL_CAST_SUCCESS", "EvasiveTactics", 1215090)
	self:Death("TorqueClankfireDeath", 237554)
