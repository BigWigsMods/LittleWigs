...
do
	...
	function mod:StartTimers()
		...
		if self:UnitBuff("boss1", self:SpellName(114929)) then -- Bloating Brew
			self:CDBar(106546, 7.0) -- Bloat
		elseif self:UnitBuff("boss1", self:SpellName(114930)) then -- Blackout Brew
			self:CDBar(106851, 6.6) -- Blackout Brew
		end

		if self:UnitBuff("boss1", self:SpellName(114931)) then -- Bubbling Brew
			self:CDBar(106563, 22.1) -- Bubble Shield
		elseif self:UnitBuff("boss1", self:SpellName(114932)) then -- Yeasty Brew (Can summon Yeasty Brew Alementals)
			self:CDBar("summon", 21.8, CL.next_add, 116155)
		end

		if self:UnitBuff("boss1", self:SpellName(114933)) then -- Sudsy Brew
			self:CDBar(-5658, 29.8) -- Wall of Suds
			self:ScheduleTimer(warnForWallOfSuds, 29.8, self)
		elseif self:UnitBuff("boss1", self:SpellName(114934)) then -- Fizzy Brew
...
do
	...
	function mod:Ferment(args)
		...
		if self:MobId(args.destGUID) == 59479 then -- players can be healed by this if they intercept the beams
			...
			if t-prev > 2 then
				prev = t
				self:Message(args.spellId, "Urgent", not self:UnitDebuff("player", args.spellName) and "Warning", CL.onboss:format(args.spellName)) -- don't annoy with sounds those who are already intercepting some
			end
		end
	end
end
