...
function mod:UNIT_SPELLCAST_SUCCEEDED(_, _, _, spellId) -- Conjure Mystery Toxin
	...
	if spellId == 93695 then -- Toxic Coagulant
		...
		self:Message(93617, "cyan", "Info", self:Healer() and L.toxin_healer_message:format(toxicCoagulant) or L.coagulant:format(toxicCoagulant))
	elseif spellId == 93563 then -- Toxic Catalyst
		local toxicCatalyst = self:SpellName(93689)
		self:CastBar(93689, 11)
		self:Message(93689, "cyan", "Info", self:Healer() and L.toxin_healer_message:format(toxicCatalyst) or L.catalyst:format(toxicCatalyst))
	end
end
